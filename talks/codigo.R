...
type <- rep(c("Women", "Men"), each = n_points)
...
model <- rq(income ~ gender + race + age + year_education, 
   tau = 2:18/20, data = dados_paraiba)
resumo_freq <- summary(model, se = 'rank')
plot(resumo_freq)

coef_lm <- 
  coef(summary(lm(income ~ gender + race + age + year_education, 
                  data = dados_paraiba))) %>% 
  as.data.frame() %>% 
  janitor::clean_names() 
...
coef_models <- 
  ...
  mutate(variable = factor(variable), 
         variable = forcats::fct_recode(variable, 
                                        "years education" = "year_education", 
...
### Estimacao bayesiana
model_bayes <- 
  bayesQR::bayesQR(income ~ gender + race + age + year_education, 
            quantile = 2:18/20, data = dados_paraiba, ndraw = 110000, 
            keep = 100)

model_bayes_2 <- lapply(2:18/20, function(a){
  R2BayesX::bayesx(income ~ gender + race + age + year_education, 
                   data = dados_paraiba,
...
plot2d(pAnosEstudo ~ year_education, data = nd, ylim = ylim)
dev.off()
...
plot2d(pAnosEstudo_sigma ~ year_education, data = nd, ylim = ylim2)
dev.off()
